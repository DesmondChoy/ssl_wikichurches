...
\midrule
Paired gap & $+$0.043 IoU, $+$0.033 Cov, $+$0.360 KL, $+$0.038 EMD & Lead is not a leaderboard artifact \\
...
moving from an earlier frozen head (\texttt{layer4/head5}) to late-layer
adapted heads under both LoRA and Full. Coverage and EMD agree on the
high-level reading, with DINO-family stability being cross-metric while MAE and
CLIP preserve only the broader adaptation pattern rather than the exact
winning head.
...
Adaptation is family-specific rather than uniform. DINOv3's
\texttt{layer10/head8} is the best frozen head not only for IoU@90 but also
for Coverage and EMD, and it remains the best head for all three metrics under
both LoRA and Full. CLIP behaves differently: its strongest frozen IoU@90 head
sits at \texttt{layer4/head5}, but its strongest adapted heads move to
\texttt{layer11}. Within \texttt{layer11}, the top frozen head is \texttt{H4};
LoRA promotes \texttt{H11}; Full promotes \texttt{H3}
(Figure~\ref{fig:q3_delta}). MAE sits between: Full preserves and strengthens
\texttt{layer10/head5} under IoU@90, while LoRA shifts the strongest IoU@90
head to \texttt{layer11/head7}.
...
Taken together, Q3 extends rather than stands apart from Q1 and Q2. Q1 showed
DINOv3 has the strongest frozen expert-aligned prior; Q2 showed adaptation
helps most when the frozen model has room to redirect attention; Q3 shows
where that behaviour appears inside the transformer. DINO-style models
preserve stable dominant heads, MAE is partially reshaped, and CLIP
reorganises from an earlier frozen head toward later adapted heads. The
specialisation is sparse, shaped by pretraining objective, and most convincing
for larger architectural structures rather than fine ornamentation.
