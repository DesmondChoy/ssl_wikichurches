...
% spconf is two-column; keep resized tables inside one real column.
\newlength{\reportcolwidth}
\setlength{\reportcolwidth}{\dimexpr\textwidth-\columnsep\relax}
\setlength{\reportcolwidth}{0.5\reportcolwidth}
...
\parbox{\columnwidth}{\small \textbf{Fig.~\thefigure:} End-to-end pipeline.
The 139-image expert-annotated evaluation set is disjoint from the fine-tuning
pool. Linear Probe is a zero-$\Delta$ control (backbone frozen); ResNet-50 is
excluded from Q2. Attention extraction depends on model family; Q3 is
restricted to native CLS-token models. Five alignment metrics are computed
against expert boxes ($M$ binary, $G$ Gaussian) and calibrated against four
naive baselines. Cached artefacts feed the Q1, Q2, and Q3 analyses.}
\end{center}
...
\begin{tabularx}{\reportcolwidth}{@{}L{0.25\reportcolwidth}L{0.37\reportcolwidth}Y@{}}
\toprule
Method & Models & Notes \\
\midrule
...
{\small
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\reportcolwidth}{@{}L{0.27\reportcolwidth}L{0.34\reportcolwidth}Y@{}}
\toprule
...
\label{tab:q1}
\resizebox{\reportcolwidth}{!}{\small
...
The authors used Claude (Anthropic) to assist with literature review
summarisation, LaTeX formatting and table layout, drafting prose for
subsequent revision, and code-comment polishing in the analysis pipeline.
All experimental design, results, and interpretations are the authors' own,
and the authors are responsible for the content and quality of the submitted
work.
