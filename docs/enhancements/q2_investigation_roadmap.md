...
- **CLIP** (Radford et al., 2021) — trained on 400M web image–text pairs with a global contrastive loss (InfoNCE softmax): $L_i = -\log\frac{\exp(\text{sim}(I_i,T_i)/\tau)}{\sum_j \exp(\text{sim}(I_i,T_j)/\tau)}$. No patch-level spatial pressure exists: the only training signal is whether the whole-image embedding matches a text embedding. CLIP's style gains concentrating on Romanesque and Gothic is explained by the caption density of those styles in web data — features like "pointed arch portal," "rose window," and "round arch" appear frequently in English descriptions of churches. Renaissance and Baroque features (Pediments, Volutes, Pilasters) have narrower English-language coverage in web alt-text. Fine-tuning adds the spatial pressure CLIP's objective never provided. Note: the style-specificity of CLIP's gain is plausible from training data distribution but is not directly quantified — a patch–text similarity probe (Step 7) would confirm it.

- **SigLIP / SigLIP 2** (Zhai et al., 2023; Tschannen et al., 2025) — both use pairwise sigmoid loss over 10B WebLI image–text pairs (patch size 16). SigLIP replaces CLIP's softmax normalization with a per-pair sigmoid, decoupling the loss from batch size (optimal at 32k) but keeping the same global alignment objective — no spatial signal. SigLIP 2 adds LocCa (spatial grounding losses, 20% of training weight) and self-distillation + masked patch prediction for the **last 20% of training**; the whitepaper reports this yields +2% ADE20k segmentation mIoU. This partial spatial stage explains SigLIP 2's higher Cohen's d (0.781 vs. SigLIP 0.604) despite similar frozen IoU — the local losses modestly increase fine-tuning responsiveness without matching DINO-class frozen alignment. Both show LP Δ = 0.000 (backbone gradient required), confirming neither develops linearly-accessible spatial representations from their global objectives alone. Neither model shows the Gothic/Romanesque-specific gain CLIP shows, consistent with their sigmoid loss not emphasizing linguistic feature names as strongly as CLIP's InfoNCE.

- **DINOv2** (Oquab et al., 2024) — trained on 142M curated images (LVD-142M) with DINO (student–teacher CLS discrimination) + iBOT patch prediction loss ($L_{\text{iBOT}} = -\sum_i p_i \log \hat{p}_i$ per masked patch against teacher targets) + KoLeo regularization ($L_{\text{KoLeo}} = \frac{1}{n}\sum_i \log d_{n,i}$, encourages uniform feature spreading), patch size 14. The whitepaper's ablation table shows each component compounds: adding KoLeo alone yields +2.3% k-NN accuracy; untying the DINO and iBOT heads adds a further +0.3%. DINOv2 achieves 75.9 mIoU on ADE20k with frozen features (vs. OpenCLIP 63.8, +12.1 points), with only a 2% gap to fine-tuning on ImageNet-1k — confirming spatial attention alignment is pretraining-baked and not unlockable by style supervision.

- **DINOv3** (Siméoni et al., 2025) — extends DINOv2 on 1.7B curated images (LVD-1689M) with a Gram anchoring loss applied during a refinement phase: $L_{\text{Gram}} = \|X_S X_S^\top - X_G X_G^\top\|_F^2$, where $X_S, X_G \in \mathbb{R}^{P \times d}$ are L2-normalized patch features of student and early teacher. The paper's ablation shows that without Gram anchoring, dense task performance degrades after ~200k iterations of long training; with it, ADE20k mIoU is maintained and reaches 81.1 (vs. DINOv2's 75.9, +5.2 points). This directly explains our frozen IoU of 0.1327 — the highest in the dataset. The small coverage drop after fine-tuning we observe (Δcov = −0.0049) is consistent with the paper's finding that CLS–patch cosine similarity rises during long SSL training and trades off against dense spatial breadth: fine-tuning sharpens selectivity while the Gram-anchored patch structure resists reorganization.
...
- **Language cluster (CLIP/SigLIP/SigLIP2, r ≈ 0.43–0.58)** — all three use global image–text alignment as their sole primary objective (InfoNCE softmax for CLIP; per-pair sigmoid for SigLIP/SigLIP 2). Because none applies patch-level spatial pressure during pretraining, their frozen attention is diffuse in a similar way — LP Δ = 0.000 for all three confirms the frozen CLS token is not linearly separable for style in any of them. Whatever images are "easy" for CLIP to improve on are also easy for SigLIP, because the barriers are identical (no pre-existing spatial routing). They improve on the same images because they share the same frozen deficiency.

- **MAE (anti-correlated with all, r ≈ −0.22 to −0.31)** — trained on ImageNet-1k with a pixel-reconstruction loss ($L = \|x - \hat{x}\|_2^2$ on masked patches only) under 75% masking — the whitepaper's ablation identifies this as the optimal ratio (76.5% fine-tune accuracy vs. 73.9% at 50% and 76.0% at 90%), precisely because the heavier mask forces the encoder to build precise local geometry representations from context. MAE has no style signal and no patch-correspondence objective. Its fine-tuning improvement targets a completely different image subset (Renaissance, with pediment-class geometry) because what makes an image "improvable" for MAE is categorically different from the language cluster: MAE responds to images where its local geometry encoding can be redirected by style gradients, not images where text-aligned patch features are latent.

- **DINO pair (DINOv2/v3, r = 0.33 with each other, near-zero with language cluster)** — both trained with iBOT (patch-level masked prediction against teacher targets) on large curated image-only datasets (LVD-142M and LVD-1689M respectively), producing frozen IoU of 0.082 / 0.133 — already at or near alignment with expert annotations. DINOv2's whitepaper shows frozen features achieve 75.9 mIoU on ADE20k (vs. OpenCLIP 63.8) with only a 2% fine-tuning gap on ImageNet, meaning style supervision adds almost nothing. DINOv3's Gram anchoring further locks patch structure, yielding 81.1 mIoU frozen on ADE20k. Neither improves meaningfully with FT on our task. The r = +0.677 between DINOv3 frozen IoU and CLIP Δ IoU is not a training configuration effect — it reflects that the same images have prominent, spatially compact annotations that any spatially-sensitive model (frozen DINO or fine-tuned CLIP) will align to.
...
1. **Pixel reconstruction rewards local geometric precision.** The decoder predicts exact pixel intensities via $L = \|x - \hat{x}\|_2^2$ (applied only to masked patches). This requires the encoder to build representations that preserve edge structure, shape boundaries, and texture at the patch level. The whitepaper's masking ratio ablation shows 75% is optimal: at 50% the task is too easy (73.9% FT accuracy) and at 90% context is insufficient (76.0%); 75% achieves 76.5% because the encoder must reason from sparse visible cues about precise missing geometry. Triangular Pediments and Broken Pediments — the two largest gainers (Δ +0.080 and +0.055) — are defined by sharp angular and curved geometric contours that are highly recoverable from adjacent visible patches. Belt Courses and Pilasters (the two features with negative Δ) are elongated, low-contrast horizontal or vertical bands with low reconstruction specificity relative to their spatial extent.

2. **No style or semantic labels during pretraining.** MAE's encoder has no concept of architectural style — all fine-tuning signal comes from the style classification task. When trained to distinguish Renaissance from the other three styles, the gradient must find the most style-discriminative regions. Pediment forms are nearly exclusive to Renaissance in this dataset; Belt Courses and Pilasters appear across multiple styles. The model learns to suppress the ambiguous features and concentrate on the discriminative ones — producing the observed negative Δ for Pilaster (−0.012) and Belt Course (−0.016) and large positive Δ for Triangular Pediment (+0.080).

3. **Linear probing fails; backbone gradients are required.** MAE's paper explicitly notes that linear probing accuracy is "weakly correlated" with fine-tuning accuracy, reporting a 7-point gap on ImageNet (76.6% LP vs. 83.6% FT for ViT-L) and a ~10-point gap on ADE20k segmentation (~48% vs. ~58%). Our data extends this: MAE LP Δ = 0.000 across all 139 images — the frozen CLS token is not style-separable at all. Only full FT or LoRA (which both flow gradients into the backbone) can reorganize spatial attention. This contrasts with DINOv2/v3, where the frozen-to-fine-tuned gap on ImageNet classification is only ~2% — frozen features are already sufficiently discriminative.
