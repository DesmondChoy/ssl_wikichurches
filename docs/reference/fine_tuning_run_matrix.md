...
The Q3 per-head study uses the architecture-native CLS-token scope:
...
The optional frozen-backbone Q3 control uses the same per-head path:
...
Generate the report and presentation assets:
...
|---|---|---|---|
| Batch selector | `outputs/results/active_experiment.json` | Yes | Tells the app and reporting scripts which batch is active |
| Split artifact | `outputs/results/experiments/<experiment_id>/splits/<split_id>.json` | Yes | Shared canonical split for the batch |
| Run manifests | `outputs/results/experiments/<experiment_id>/manifests/` | Yes | One manifest per `model × strategy` run |
| Run matrix | `outputs/results/experiments/<experiment_id>/run_matrix.json` | Yes | Single source of truth for figures/docs |
| Batch ledger | `outputs/results/experiments/<experiment_id>/fine_tuning_results.json` | Yes | Accumulates every run in the batch |
| Checkpoints | `outputs/checkpoints/<experiment_id>/` | No | Trained weights |
| Q2 analysis | `outputs/results/experiments/<experiment_id>/q2_metrics_analysis.json` | Yes | Consumed by `/api/metrics/q2_summary` and `/q2` |
| Q2 image-level deltas | `outputs/results/experiments/<experiment_id>/q2_delta_iou_analysis.json` | Yes | Consumed by `/api/metrics/q2_image_deltas` and legacy delta-only consumers |
| Q3 per-head metrics | `outputs/cache/metrics.db` (`head_image_metrics`, `head_summary_metrics`, `head_feature_metrics`, `head_feature_image_metrics`) | No | Consumed by Dashboard Q3, `/q3-report`, and Image Detail Q3 |
| Q3 per-head attention | `outputs/cache/attention_viz.h5` (`*_head<h>` variants) | No | Consumed by Q3 raw-attention and drill-down views |
| Per-style Q2 analysis | `outputs/results/experiments/<experiment_id>/style_breakdown.json`, `style_breakdown.png` | Yes | Generated by `analyze_style_breakdown.py` |
| Cross-model Q2 correlation | `outputs/results/experiments/<experiment_id>/model_correlation.json`, `model_correlation_scatter.png`, `model_correlation_heatmap.png` | Yes | Generated by `analyze_model_correlation.py` |
| Feature-level Q2 delta analysis | `outputs/results/experiments/<experiment_id>/feature_delta_iou_mae_full_renaissance.json`, `feature_delta_iou_mae_full_renaissance.png` | Yes | Generated by `analyze_feature_delta_iou.py` |
| Legacy fallback summary | `outputs/results/q2_metrics_analysis.json` | Yes | Repository-level fallback path when the active pointer is absent |
| Legacy fallback image-level deltas | `outputs/results/q2_delta_iou_analysis.json` | Yes | Repository-level fallback path when the active pointer is absent |
| Run-matrix figures | `outputs/figures/` | Yes | Figure-generation outputs driven by the active experiment |
...
The large app caches remain intentionally untracked: `outputs/cache/` contains attention HDF5 files, rendered heatmaps, feature tensors, `metrics.db`, and `metrics_summary.json`. The final report's Q1 and Q3 Appendix A pointers rely on those cache files after regeneration, while the checked-in `outputs/results/` artifacts preserve the Q2 experiment provenance and headline analysis results.
